...
## النشر على Netlify (نسخة الويب)

قاعدة البيانات المحلية (`hive_ce_flutter`) تدعم الويب عبر IndexedDB، والكود
مُجهَّز مسبقًا لفصل ميزة الطباعة الحرارية عبر بلوتوث (غير مدعومة على الويب)
عن باقي التطبيق تلقائيًا وقت الترجمة (`lib/services/print_service.dart`).
لذلك يمكن نشر نسخة ويب تجريبية من التطبيق تلقائيًا على Netlify عبر:

```
.github/workflows/deploy-netlify.yml
```

البناء نفسه يحدث داخل GitHub Actions (نظام بناء Netlify لا يحتوي Flutter
SDK أصلًا)، ثم يُرفَع مجلد `build/web` الجاهز مباشرة عبر Netlify CLI —
دون أي ربط مباشر بين مستودع Netlify والمستودع على GitHub.

### ملاحظة: ميزة لا تعمل على الويب

**الطباعة الحرارية عبر بلوتوث** تُخفى تلقائيًا من شاشة الإعدادات على الويب
(فحص `kIsWeb`)، لأن بلوتوث الكلاسيكي غير مدعوم داخل المتصفح أصلًا. باقي
الميزات (الفواتير، PDF، التوقيع، النسخ الاحتياطي...) تعمل على الويب بشكل
طبيعي.

### 1) أنشئ موقعًا فارغًا على Netlify (مرة واحدة فقط)

1. من [app.netlify.com](https://app.netlify.com) اختر **Add new site ←
   Deploy manually**، واسحب أي مجلد/ملف مؤقت لإنشاء الموقع لأول مرة (سيُستبدل
   تلقائيًا بأول نشر من GitHub Actions).
2. من **Site configuration ← General ← Site details** انسخ **Site ID**.

### 2) ولّد رمز وصول شخصي (Personal Access Token)

من **User settings ← Applications ← New access token** في حساب Netlify،
ثم انسخ الرمز الناتج فورًا (لن يظهر مرة أخرى).

### 3) أضف الأسرار (Secrets) في إعدادات مستودع GitHub

من **Settings → Secrets and variables → Actions → New repository secret**
أضف سرَّين بهذين الاسمين بالضبط:

| اسم السر             | القيمة                     |
|------------------------|------------------------------|
| `NETLIFY_AUTH_TOKEN`   | الرمز من الخطوة 2            |
| `NETLIFY_SITE_ID`      | Site ID من الخطوة 1          |

### 4) شغّل النشر

أي `push` جديد لفرع `main` يُشغّل workflow **"نشر نسخة الويب على
Netlify"** تلقائيًا وينشر مباشرة على رابط الإنتاج (`--prod`)، أو شغّله
يدويًا من تبويب Actions. رابط الموقع النهائي يظهر في نهاية سجل خطوة "النشر
على Netlify" داخل التشغيلة (Website URL).
